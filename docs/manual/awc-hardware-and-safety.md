...
three methods (continuous trickle, batch-simultaneous, batch drain-then-fill), on a
fully-flexible schedule (litres **or** %, per day **or** week), or on demand
("change N litres now").

## What you need (v1)

- **ESP32 + two peristaltic pumps** (standard DC for v1; steppers later) exposed to
...
| Net-imbalance tracking | Logs cumulative drain-vs-fill; warns (and can auto-trim) before salinity drifts. |
