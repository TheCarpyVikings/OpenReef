...
> **Update (0.7.185):** the reef-buddy mascot and the Lagertha avatar were removed from the
> product and from this site — the floating guide, the pose art, the `apex-throne` reveal and
> the deep-dive inline buddy are all gone. The **Cheeky/Professional tone** and the calm-only
> anti-Apex jokes stay; where the plan below says "buddy bubble", the voice now speaks in the
> page's own copy (deep dives carry it as a pull-quote). Everything else in this plan stands.
...
6. **Features shelf** — 2D cards with the real demo webps (mission control, sensors,
   ...
   and camera AI. Real product screenshots > renders.
...
- DONE 2026-07-25: all 10 feature screenshots captured from the live HA integration
  (+ click-to-zoom lightbox); UK prices verified against All Things Aquatic /
...
was photobombed by the guided tour — since 0.7.185 there is no tour to skip,
so the harness no longer pre-sets anything.

**Live demo (/demo/, "The Showroom Reef")** — shipped 2026-08-04. The REAL
panel (openreef-panel.js copied verbatim, pinned per release) mounted on the
marketing site against a browser hass shim (site/src/demo/shim.ts): recorded
fixtures answer the read WS commands, a deterministic ocean model breathes the
sensors every 5 s and synthesises trend history (pure value(t) functions, so 1 h
and 30 d windows agree), interactive writes (toggle/save/manual tests/settings)
mutate session-local config, unarmed equipment refuses with the real safety
message, hardware actions throw a friendly "needs a real tank". Opens with the
scripted "2 AM save" incident (site/src/demo/Opener.tsx, skippable, once per
visitor, replay button in banner) whose event is also seeded in the demo's
alert history three nights back. Fixtures are generated by the ACTUAL backend
handlers running in the tests/ fake-HA harness (`pnpm demo:fixtures` →
site/tools/demo-fixtures.py; also copies the panel). Swap in the
real tank later with `HA_URL=… HA_TOKEN=… pnpm demo:record`
(site/tools/demo-record.mjs, read-only commands only; REVIEW output before
committing — it becomes public). Timestamps rebase in-browser so fixtures never
go stale. Banner honesty: "seeded data, live tide" until a recording replaces it.
