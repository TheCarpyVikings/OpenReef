...
### 8.8 Stage G as built (0.7.204, 2026-09-17)

- **One text rendition** (`report.text_blocks` → `markdown` / `plain_text`): every section the viewer shows, in its order, from the compiled report only — the shared text and the viewer never disagree. Water rows name the source of each latest figure. `share_chunks` splits at blank lines (then line ends) under Telegram's limit.
- **Copy as text**: `report_compile` carries `markdown`; the panel hands it to the clipboard.
- **Share card PNG**: `_reportShareCardSvg` — a standalone SVG with inline styles (period, score coloured by band, condition · consistency, Reef Health and its move, the verdict, the counts, the top recommendation, the Reef-Health-by-day line); rasterised on a canvas at 2× to **Save PNG** (download) or **Copy image** (clipboard, where the browser allows). Nothing external is loaded, so the canvas is never tainted.
- **Print**: a standalone light document in a new window (`_reportPrintDocument`) built from the same section renderers with a print stylesheet — buttons and controls hidden, the score's why-lines open, photos left out (their URLs need the panel's session) — then `window.print()`.
- **Send** (`openreef/report_share {period, which, anchor, readings, target}`): compiles the way the viewer saw it (the panel's readings ride along), renders plain, sends to the maintenance reminders' push target (or the one named) in ordered, blocking chunks; a Telegram target is asked for `parse_mode: plain_text`; a target that refuses the data block gets the message plain; logged on the activity (`kind="report"`). No target → `no_target`, never guessed.
- **Pulse card** (`report-week`): "Last week: 78 ↑" with the stored week's verdict, from `_reportLastWeek()` — the stored weekly snapshot first (its Week Score and the week before's), else last week's average Reef Health stamp; the Home hero card reads the same helper.
