...
def test_last_seen_and_missing_semantics_survive_restart():
    ...
    # restart: fresh runtime rehydrated from the flushed summary
    flushed = vision.summary(rt, 2000.0)
    rt2 = vision.new_runtime(SPECIES, ZONES, now=500000.0)
    vision.rehydrate(rt2, flushed)
    assert rt2["lastSeen"]["clownfish"] == 2000.0
    assert rt2["onlineAt"] == 1000.0  # oldest online time survives
    # clownfish seen long ago -> missing; goby NEVER seen but vision has been
    # online past the threshold -> also missing (the restart-amnesia fix)
    missing = vision.missing_species(rt2, now=500000.0, threshold_hours=12)
    assert "clownfish" in missing and "chalk_goby" in missing
    # threshold 0 = alert disabled
    assert vision.missing_species(rt2, 500000.0, 0) == []
    ...
    vision.rehydrate(rt3, "corrupt")
    vision.rehydrate(rt3, {"lastSeen": "nope", "onlineAt": []})
